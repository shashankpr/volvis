...
public class RaycastRenderer extends Renderer implements TFChangeListener {
    ...
    private void intersectFace(double[] plane_pos, double[] plane_normal,
      ...
      int traceRayMIP(double[] pixelCoord, double[] entryPoint, double[] exitPoint, double[] viewVec, double sampleStep) {
        ...
        */
        ...
        double start = Math.max(entryPoint[0], Math.max(entryPoint[1], entryPoint[2]));
        double end = Math.min(exitPoint[0], Math.min(exitPoint[1], exitPoint[2]));
       
        if(start < end){
            voxelCoord[0] = pixelCoord[0] + (end * viewVec[0]);
            voxelCoord[1] = pixelCoord[1] + (end * viewVec[1]);
            voxelCoord[2] = pixelCoord[2] + (end * viewVec[2]);
        
                    
        for(double u = end; u > start; u -= sampleStep){
            int voxelIntensity = volume.getVoxelInterpolate(voxelCoord);
        ...
        finalPixelColor = (c_alpha << 24) | (c_red << 16) | (c_green << 8) | c_blue;
        }
        else{
            finalPixelColor = (1 << 24);
        }
...
//                
                if ((entryPoint[0] > -1.0) && (exitPoint[0] > -1.0)) {
                   ...
                   if(mipMode) 
                        pixelColor= traceRayMIP(pixelCoord, entryPoint,exitPoint,viewVec,sampleStep);
